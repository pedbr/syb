    ...
    this.socket.onmessage = function onMessage(msg) {
      ...
      switch (parseInt(msg.data.charAt(i++), 10)) {
        case 0: // OPEN
          ...
          return
        case 4: // MESSAGE
          type = parseInt(msg.data.charAt(i++), 10) // eslint-disable-line no-unused-vars
          namespace = '/'
          if (msg.data.charAt(i) === '/') {
            const iNamespace = i
            while (i < msg.data.length && msg.data.charAt(++i) !== ',');
            namespace = msg.data.substring(iNamespace, i++) // eslint-disable-line no-unused-vars
          }
